$(function(){
	$("#brand").change(function(){
		...
		switch ($("#brand").val()) {
		case "nokia":
			$("#selectedBrand").html("诺基亚");
			Page=1;
			getXml($("#brand").val());
			$("#wrap").show();
			$("#selectedType").html("");
			$("#imgType").attr("src", "../img/mobilephonetype.gif");
			$("#imgBrand").attr({"src":"../img/nokia/nokia.gif","alt":"诺基亚","title":"诺基亚"});
			...
			$("#type").append("<option value='w240h320' >屏幕240*320</option>");
			
			break;
		case "moto":
			$("#selectedBrand").html("摩托罗拉");
			Page=1;
			getXml($("#brand").val());
			$("#wrap").show();
			$("#selectedType").html("");
			$("#imgType").attr("src", "../img/mobilephonetype.gif");
			$("#imgBrand").attr({"src":"../img/moto/moto.gif","alt":"摩托罗拉","title":"摩托罗拉"});
			...
			$("#type").append("<option value='w240h320' >屏幕240*320</option>");
				break;
		case "sony":
			$("#selectedBrand").html("索爱");
			Page=1;
			getXml($("#brand").val());
			$("#wrap").show();
			$("#selectedType").html("");
			$("#imgType").attr("src", "../img/mobilephonetype.gif");
			$("#imgBrand").attr({"src":"../img/sony/sony.gif","alt":"索爱","title":"索爱"});
			...
			break;
		case "samsung":
			$("#selectedBrand").html("三星");
			Page=1;
			getXml($("#brand").val());
			$("#wrap").show();
			$("#selectedType").html("");
			$("#imgType").attr("src", "../img/mobilephonetype.gif");
			$("#imgBrand").attr({"src":"../img/samsung/samsung.gif","alt":"三星","title":"三星"});
		...
		break;
		case "android":
			$("#selectedBrand").html("(android)安卓");
			Page=1;
			getXml($("#brand").val());
			$("#wrap").show();
			$("#selectedType").html("");
			$("#imgType").attr("src", "../img/mobilephonetype.gif");
			$("#imgBrand").attr({"src":"../img/android/android.png","alt":"(android)安卓","title":"(android)安卓"});
			...
			$("#type").append("<option value='DOPOD A6288' >多普达  A6288</option>");
			...
			$("#type").append("<option value='HUAWEI U8220' >华为 U8220</option>");
			$("#type").append("<option value='SHOUPAI A60' >首派 A60</option>");
			$("#type").append("<option value='Google Nexus One' >谷歌 Nexus One</option>");
			$("#type").append("<option value='SamSung GT-i9000' >SamSung GT-i9000</option>");
			$("#type").append("<option value='AIGO E700' >爱国者E700</option>");
			$("#type").append("<option value='HUAWEI C8600' >华为C8600</option>");
...
$(function(){
	$("#type").change(function(){
		...
		var urlSuffix = " ><img src='../img/an-ljxz.jpg' title='立即下载' /></a>";
		var androidContent="";
		var urlNotFound = "<span onclik='alert(" + "'没有找到！')" + "'>" +
		                  "<img src='../img/an-ljxz.jpg' title='立即下载' /></span>";	
...
function getXml(xml_name){ 
	$.ajax({
        ...
        error:function(xml){
			$("#TopInfo").html("读取失败");
		    $("#Content").html("<div style='padding:100px 0;text-align:center;'>读取失败</div>");},  
        async: false , 
		success:function(xml){ 
    		...
    		for(var iData = 0; iData < datas.length; iData++) {
    		  ...
    		  if(!(Page <= Pages)) Page = Pages;
    		  $("#TopInfo").html("共有<strong>" + $(xml).find("data").length + "</strong>张图片&nbsp;&nbsp;当前显示 第" + BeginNO + "-" + EndNO + "篇");

    		  //分页
    		  if(Page != 1){
    			  Temp ='<a href="javascript:void(0)" onclick="GotoPage(1)">&lt;&lt;首页</a> <a href="javascript:void(0)" onclick="GotoPage(' + (Page - 1) + ')">上页</a>&nbsp;';
    			  }else{
    			  Temp = '&lt;&lt;首页上页&nbsp;';}
    		  ...
    		  if(EndNO == 0){
    		      $("#Content").html('<div style="padding:100px 0;text-align:center;">暂无图片</div>');
    		      return;
    		  }
