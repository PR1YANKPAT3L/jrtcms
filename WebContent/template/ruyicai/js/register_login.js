...
regArray[1]= "/rules/emailRegisterIndex.html";//邮箱注册第一个页面***
	...
	/**if($("#checkCard").hasClass("Switch")){
		//得到页面上点卡的信息
		 var params = "cardid="+$("#cardid").val()+"&cardpwd="+$("#cardpwd").val();
			// 发送请求
			$.ajax({
				url	: '/chlw/function/tuserinfoAction!getChannel',
				type:"POST",//数据发送方式
				data:params,//参数
					...
					if(json =="2"){
						...
						if ($("#registerXieyi").attr("checked") != true && $("#registerXieyi").attr("checked") != "checked") {	
					 	...
					 	if( url == "key=2"){
							...
							// 发送请求
							$.ajax({
								url	: '/chlw/function/tuserinfoAction!ryc_Register',
								type:"POST",//数据发送方式
								data:params,//参数
								dataType:'text',//接受数据格式
								async: false, 
...
function getSend(){
	if(pageFlashNum==0){
		pageFlashNum++;
		$.ajax({
			url : '/chlw/function/tuserinfoAction!sendMessage',
			type:"POST",//数据发送方式
			dataType:'html',//接受数据格式
			success:function(data){
				sendCount=data;
...
function getEmailSend(){
	if(pageFlashNum==0){
		pageFlashNum++;
	$.ajax({
		url : '/chlw/function/tuserinfoAction!sendEmailMessage',
		type:"POST",//数据发送方式
		dataType:'html',//接受数据格式
		success:function(data){
		sendCount=data;
...
function PhoneCheck(url) {
	...
	$.ajax({
		url	: '/chlw/function/tuserinfoAction!phoneCheck',
		type:"POST",//数据发送方式
		data:params,//参数
		dataType:'html',//接受数据格式
		success:function(data){
