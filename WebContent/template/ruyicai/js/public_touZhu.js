...
function splitZhuma(zhuma) {
	...
	if ($("#caiZhong").val() == "dlt" && zhumaStr.length == 5) {
		...
		//alert((zhumaStr[0]+","+zhumaStr[1]+","+zhumaStr[2]).length);			
		...
		success: function(msg) {
			...
			if (msg.end_time != null && msg.end_time != "") {
				...
				if (jirikaijiangriqi.indexOf(nowDate) > -1) { //是否当前时间 
					$("#jinrikaijiangId").html("<img src='/images/jrkaijiang.gif' />");
				}
			}
			//获取遗漏
			var wanfa = "";
...
function getZhumaView(lotteryNumber) {
	...
	if (lotno == 'F47104') {
		...
		if ($("#sub_nav_1").is(":visible")) {
			...
			var danCodes = codes[0].split("$");
			var tuoCodes = codes[1].split("$");
			var qian_dan = danCodes[0].split(",");
			var qian_tuo = danCodes[1].split(",");
			var hou_dan = tuoCodes[0].split(",");
			var hou_tuo = tuoCodes[1].split(",");
...
function faqiHemai_jc() {
	...
	if ((buyAmt == "" || buyAmt == "0") && (safeAmt == "" || safeAmt == "0")) {
		...
		$("#buyAmt").focus();
		return false;
	} else if (parseInt($("#investField_standrad").html())<6) {
		...
		$("#minAmt").focus();
		return false;
	} else if ($("#isMinAmt").is(':checked') && minAmt > (parseInt($("#investField_standrad").html()) / 2)) {
...
function bili_jc(idstr) {
	...
	a = Number(a);
	var b = Number($("#investField_standrad").html());
...
function checkDesc() {
	var desc = $("#description").val(); // 合买宣传语
	if(desc.indexOf("script")>0||desc.indexOf("<")==0 ||desc.indexOf("%")==0){
		openAlert(decodeURI(EncodeUtf8("方案宣传不能出现特殊字符")));
		return false;
	}
	if (desc.length > 500) {
				...
				if(single!=""||single.length!=0){
					...
					if(!telePattern.test(single)){
						...
						if(userRightList.indexOf(single)!=-1){
							...
							//alert("key:"+key+";abc:"+abc+";abc.length:"+abc.length);
...
//统计自购的金额和倍数
function ownMessage(){
	var ownMultiple = Number($("#ownMultiple").val());
	var investField = Number($("#investField").html());
	ownMoney = ownMultiple * investField;
	//disabled
	if($("#ownCheck").attr('checked') != 'checked'){
