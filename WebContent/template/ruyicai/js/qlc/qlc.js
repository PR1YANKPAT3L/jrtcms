...
function danTuo_selectBall(BallColor, BallNum, BallType) {
	...
	$("#x_danma").html(getBallCount("r", "danMa", 30));
	$("#x_tuoma").html(getBallCount("r", "tuoMa", 30));
	return;
}
		...
		if (BallColor == 'r' && (BallType == 'danMa' || BallType == 'tuoMa')) {
			if (BallType == 'danMa') {
				if (getBallCount("r", "danMa", 30) > 5) {
					openAlert(decodeURI(EncodeUtf8("红球最多只能选择 6个胆码！")));
					return false;
				}
			}
...
function ClearCheck() {
	for (var i = 1; i <= 30; i++) danTuo_setBallState("r", danTuo_getBallNum(i), false, "danMa");
	for (var i = 1; i <= 30; i++) danTuo_setBallState("r", danTuo_getBallNum(i), false, "tuoMa");
	$('#dt_x_zhu').html("0");
	$('#dt_x_yuan').html("0");
	$('#x_danma').html(0);
	$('#x_tuoma').html(0);
...
function GetLotteryInvestNum() {
	...
	if ((RedCount < 7)) return 0;

	var InvestNum = 1;

	for (var i = 7; i <= RedCount; i++) InvestNum = zuhe(7, RedCount);
...
function qlc_redRand() {
	for (var i = 1; i <= 30; i += 1) SetBallState("r", GetBallNum(i), false);
...
function ClearSelect() {
	for (var i = 1; i <= 30; i++) SetBallState("r", GetBallNum(i), false);
	$("#x_zhu").html("0");
	$("#x_yuan").html("0");
}
...
function btn_2_RandManyClick_dd(n) {
	//胆码个数判断
	if (getUserSelectBall('r', 30, '').length < 1) {
		openAlert(decodeURI(EncodeUtf8("请选择至少选择一个胆码！")));
		return false;
	}
	if (getUserSelectBall('r', 30, '').length > 6) {
	...
	for (var i = 0; i < parseInt(n); i++) {
		//提取选中的胆码
		var redBallNumCodes = getUserSelectBall('r', 30, '');
		//调用算球的方法随机生成注码
		var finalBall = getRandBall('td_', redBallNumCodes, 7, 'r', 30, 'ball_num', 'ball_num_sh', 6, '');
		//拼接显示注码和传递给后台的注码
