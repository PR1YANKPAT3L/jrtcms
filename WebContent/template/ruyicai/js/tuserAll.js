...
function testName(){
	...
	$.ajax({
		url:'/chlw/user/tuserinfoAction!testUserName?'+params,  //后台处理程序
		type:'post',   
		dataType:'json',   
...
function testcard(){
	...
	for(b in $.browser) if(b=='msie'){params=encodeURI(params)+"&msie=1";};
	$.ajax({
		url:'/chlw/user/tuserinfoAction!testPersonId',  //后台处理程序
		type:'post',    //数据发送方式 
		data:params, 
		dataType:'json',
		success:function(data){
...
function testbank(){
	...
	if(bankname=="" || bankname=="undefined"){
		...
		//openAlert(decodeURI(EncodeUtf8("请选择银行名称！")));
		return false;
	}
...
function jumpNextTest(){
	if(t_name=="1" && t_card=="1" && checkDocumentAddress()  && cardNumberCheck() && testbank()){
		...
		for(b in $.browser) if(b=='msie'){params=encodeURI(params)+"&msie=1";};
		$.ajax({
			url:'/chlw/user/tuserinfoAction!getDnaParams_first', 
			type:'post', 
			data:params, 
			success:function(data){
				if(data=="success"){
...
function testpassword(){
	...
	$.ajax({
		 url:'/chlw/user/tuserinfoAction!testPassword?password='+password,  //后台处理程序
         type:'post',    //数据发送方式
...
function jumpthreeUrl(){
	if(amountCheck() && telCheck() && (passwordMark==1)){
		...
		for(b in $.browser) if(b=='msie'){params=encodeURI(params)+"&msie=1";};
		$.ajax({
			url:'/chlw/user/tuserinfoAction!getDnaParams_sescond',  //后台处理程序
			type:'post',    //数据发送方式 
			async:false,
			data:params, 
			success:function(data){
...
function bankRadioCharge(radioID){
	...
	var c = $("#"+radioID).val();
     if(c=='minShengBanks'){
        ...
        // $("#minshengNo").html(decodeURI(EncodeUtf8("本次充值由"))+" <font class='blue' id='dis'>"+decodeURI(EncodeUtf8("中国民生银行"))+"</font>"+decodeURI(EncodeUtf8(" 提供接口支持")));

     }else if(c=='0600'|| c=='0602'){
		 $("#dis").html(decodeURI(EncodeUtf8("上海银联电子支付")));

         //$("#minshengNo").html(decodeURI(EncodeUtf8("本次充值由"))+" <font class='blue' id='dis'>"+decodeURI(EncodeUtf8("上海银联电子支付"))+"</font>"+decodeURI(EncodeUtf8(" 提供接口支持")));
	 
	 }else if(c=='yeepay'){

         $("#dis").html(decodeURI(EncodeUtf8("易宝支付")));

         //$("#minshengNo").html(decodeURI(EncodeUtf8("本次充值由"))+" <font class='blue' id='dis'>"+decodeURI(EncodeUtf8("易宝支付"))+"</font>"+decodeURI(EncodeUtf8(" 提供接口支持")));

     }else{

         $("#dis").html(decodeURI(EncodeUtf8("支付宝")));

        // $("#minshengNo").html(decodeURI(EncodeUtf8("本次充值由"))+" <font class='blue' id='dis'>"+decodeURI(EncodeUtf8("支付宝"))+"</font>"+decodeURI(EncodeUtf8(" 提供接口支持")));
...
function checkMinsheng(){
	...
	var bankMoney = $("#minshengMoney").val();
...
function ChargeTruePage(){
	...
	if(bankValue=="zfbyy"){
		if($("#isMobile").val() == null  || $("#isMobile").val().trim() == '' || $("#isMobile").val() == 'null'){
			openAjaxPopup("openTishi",1000,"/rules/chargeTishi.html");
			return ;
		}
	}
...
function putBankUrl(){
	...
	var appendText = '<div class="AlertWindow BalanceWindow WindowCenter" ><div class="WindowTittle"><h3>'+decodeURI(EncodeUtf8("完成充值"))+'</h3><span class="Alertclose" onclick="closeChargeTishi();">&#160;</span></div><div class="InsideBorder" style="padding-top:30px;"><table class="BalanceTable"><thead><tr><th width="100">&#160;</th><td style="font-size:15px; line-height:40px; font-weight:bold;">'+decodeURI(EncodeUtf8("请您在新打开的网上银行页面上完成充值"))+'</td></tr><tr><th>&#160;</th><td>'+decodeURI(EncodeUtf8("充值完成前请不要关闭此窗口，"))+'<br />'+decodeURI(EncodeUtf8("完成充值后请根据您的情况点击下面的按钮；"))+'<br />'+decodeURI(EncodeUtf8("或者"))+'<a href="./user.html?key=4" style="color:#1f376D;">'+decodeURI(EncodeUtf8("选择其他支付方式"))+'</a>。</td></tr></thead><tbody><tr><th colspan="2">&#160;</th></tr><tr><th colspan="2"><a class="BaseBtn" href="./user.html?key=6">'+decodeURI(EncodeUtf8("完成充值"))+'</a><a class="BaseBtn" href="/rules/customesService.html" target="_blank">'+decodeURI(EncodeUtf8("遇到问题"))+'</a></th></tr></tbody></table></div></div>';
...
function closeChargeTishi(){
	...
	window.location.href="./user.html?key=6";
}

function ajaxMinshengFill(bankValue,amt,ladderflag){
	$.ajax({
		 url:'/chlw/user/tuserinfoAction!minshengCharge?radiobutton='+bankValue+'&chargeBean.transaction_money='+amt+"&ladderflag="+ladderflag, 
		 type:'post',    //数据发送方式
...
/**
function minshengCharge_Result_old(json){ //回传函数实体，参数为XMLhttpRequest.responseText
	if(json.flag){ 
		if(json.bankOrSound == 'minShengBanks'){
			$("#minshengNo").html("<form name='myForm' target='colors123' method='post'  action='"+json.objMinsheng.requrl+"'>" +
			"<input type='hidden' name='orderinfo' id='orderinfo' value='"+json.objMinsheng.msBank_Mac+"'/>" +
			"</form>" +
			"<script type='text/javascript'>" +
			"document.myForm.submit();</script>");
		}else if(json.bankOrSound == 'shangHaiBanks'){
			$("#minshengNo").html("<form name='myForm' method='post' target='colors123' action='"+json.objShangHai.pay_url+"'></form>" +
			"<script type='text/javascript'>document.myForm.submit();</script>");
		
		}else if(json.bankOrSound == 'yinLianBanks'){
			$("#minshengNo").html("<form name='myForm' method='post'  action='"+json.objYinLian.pay_url+"'></form>" +
			"<script type='text/javascript'>document.myForm.submit();</script>");
		
		}else if(json.bankOrSound == 'yeepay'){
				 $("#minshengNo").html("<form name='myForm' method='post' action='"+json.jsonYeepay.requrl+"'></form>" +
					"<script type='text/javascript'>document.myForm.submit();</script>");
		}else if(json.bankOrSound == 'zfbzx'){
			 $("#minshengNo").html("<form name='myForm' method='post'  action='"+json.jsonAlipay.requrl+"'></form>" +
				"<script type='text/javascript'>document.myForm.submit();</script>");
		}else{
			repeat('minshSubmit',decodeURI(EncodeUtf8('立即充值')));
			openAlert(json.message);
...
function chargeCardNoCheck(){
	var cardNo = $("#cardNoPoint").val();//document.getElementById("cardNoPoint").value;
...
function ajaxPointFill(){
	...
	var params=$('#pointForm').serialize(); //这里直接就序列化了表单里面的值；直接获取到表单元素的值
	    ...
	    $.ajax({
	         url:'/chlw/user/tuserinfoAction!pointCharge',  //后台处理程序
	         type:'post',    //数据发送方式
	         dataType:'json',   //接受数据格式
	         data:params,   //要传递的数据;就是上面序列化的值
...
function ajaxDnaFill(){
	 ...
	 for(b in $.browser) if(b=='msie'){params=params+"&msie=1";}
	    $.ajax({
	         url:'/chlw/user/tuserinfoAction!dnaCharge',  //后台处理程序
	         type:'post',    //数据发送方式
	         dataType:'json',   //接受数据格式
	         data:params,   //要传递的数据;就是上面序列化的值
...
function findBalance(){
	...
	{
		$.ajax({   
				url:'/chlw/user/tuserinfoAction!ajaxFindAccount',  
				type:'post', //数据发送方式   
				dataType:'json',
			...
			$.ajax({
				...
				success:function(json){
					if(json =="1"){
						...
						$.ajax({
							url:'/chlw/function/tuserinfoAction!changeUserName',
							type:"POST",
							data:para,
							async:false,
							dataType:'json',
...
//			params =params+"&nickname="+nickname+"&name="+name+"&cardid="+cardid+"&tel="+mobile+"&email="+email+"&flag=userinfo";
			if(nickflag==1 && realflag==1 && certid==1&& emailflag==1){
				$.ajax({
					url	: '/chlw/function/tuserinfoAction!modifyUserInfo',
					type:"POST",
					data:params,
					dataType:'text',
					async: false, 
					...
					success:function(json){
						if(json=="success"){
							openAlert("保存用户信息成功");
							window.location.href='/rules/user.html?key=33';
						}else if(json=="failed"){
							openAlert("修改用户信息失败！");
						}else{
							openAlert(json);
...
function checkMoney(){
	...
	if(money==null || money == ""){
		mno.innerHTML=decodeURI(EncodeUtf8("请输入提现金额！"));
		mno.style.color="#FF0000";
		return false;
	}
	if(! moneyPattern.test(money)){
		mno.innerHTML=decodeURI(EncodeUtf8("金额必须是大于0的数字！"));
		//$("#withdrawel_money").attr("value",'');
		mno.style.color="#FF0000";
		return false;
	}
	
	if(patt.test(money)){
		mno.innerHTML=decodeURI(EncodeUtf8("提现金额不可超过2位小数！"));
		//$("#withdrawel_money").attr("value",'');
		mno.style.color="#FF0000";
		return false;
	}
	if(parseInt(money)>parseInt(Valid_amount)){
		mno.innerHTML=decodeURI(EncodeUtf8("提现金额大于可提现金额！"));
		//$("#withdrawel_money").attr("value",'');
		mno.style.color="#FF0000";
		return false;
	}
	...
	if(Number(Valid_amount)>10 && Number(money)<10){
		mno.innerHTML=decodeURI(EncodeUtf8("可提现金额大于10元时，每次提现最少10元！"));
		//$("#withdrawel_money").attr("value",'');
		mno.style.color="#FF0000";
		return false;
	...
	if(Number(Valid_amount)<10 ){
		if(Number(Valid_amount)!=Number(money)){
			mno.innerHTML=decodeURI(EncodeUtf8("可提现金额小于10元时，请一次性提清！"));
			//$("#withdrawel_money").attr("value",'');
			mno.style.color="#FF0000";
			return false;
		}
	...
	if(money<=0){
		mno.innerHTML=decodeURI(EncodeUtf8("提现金额必须大于0！"));
		//$("#withdrawel_money").attr("value",'');
		mno.style.color="#FF0000";
		return false;
	}
	...
	function checkBankName()//验证开户行名称
	{
		...
		{
			$("#bankNo").html("开户行名称必须输入！");//.innerHTML=decodeURI(EncodeUtf8("开户行名称必须输入！"));
			$("#bankNo").css("color","#FF0000");//bankNo.style.color="#FF0000";
			return false;
	    }
	...
	function checkBankNO()// 验证银行账号
	{
		...
		{
			bno.innerHTML=decodeURI(EncodeUtf8("请您输入银行卡账号！"));
		...
		if(!bankNoPattern.test(bankNo)){
			bno.innerHTML=decodeURI(EncodeUtf8("请你输入正确的银行账号！"));
			bno.style.color="#FF0000";
			return false;
		}
		bno.innerHTML = "";
	...
	function checkName_real_new(){
	    //var hiddenname = $("#hiddenrealname").val().trim();
		...
		if(nameReal.length >0){
			...
			for(;j<=nameReal.length;i++,j++){
				if(reg.indexOf(nameReal.substring(i,j))>-1){
					uname.innerHTML = decodeURI(EncodeUtf8("开卡人姓名不能包含特殊字符！"));
		...
		if(nameReal==null || nameReal==""){
			uname.innerHTML = decodeURI(EncodeUtf8("开卡人姓名必须输入！"));
			uname.style.color="#FF0000";
			//alert(uname);
			return false;
		}
		if(nameReal!=null && nameReal!=""){
			if(Length(nameReal)>30){
				uname.innerHTML = decodeURI(EncodeUtf8("开卡人姓名不能超过30个字符。"));
				uname.style.color="#FF0000";
				return false;
			}
		}
	...
	function checkName_real(){
		...
		$.ajax({
			url:'/chlw/user/tuserinfoAction!changeRealName',
			type:'POST',
			data:parameters,//参数
			async:false,
			...
			success:function(data){
				if(data=="false"){
					uname.innerHTML = decodeURI(EncodeUtf8("真实姓名与身份证绑定姓名不相符，请重新输入！"));
					uname.style.color="#FF0000";
					flag=1;
					//return false;
				}
	...
	function checkAll(){
		...
		notRepeat('tikuan_botton','tikuanzhong_botton','');
		 $.ajax({
			//url:'/chlw/user/tuserinfoAction!cash?cashBean.withdrawel_money='+money+'&cashBean.ID_number='+bankNo+'&cashBean.real_name='+encodeURI(encodeURI(nameReal.value))+'&cashBean.bank_name='+encodeURI(encodeURI(bankName.value)),
			url:'/chlw/user/tuserinfoAction!cash',
			type:'POST',
			error:function(){
				openAlert("提现失败！");
	...
	function checkAllUpdate(){
		...
		//alert(money+bankName+bankNo+nameReal);
		if(money!=""&&bankName!=""&&bankNo!=""&&nameReal!=""){
			 $.ajax({
				url:'/chlw/user/tuserinfoAction!doUpdateCashDetail?cashBean.withdrawel_money='+money+'&cashBean.ID_number='+bankNo+'&cashBean.real_name='+encodeURI(encodeURI(nameReal.value))+'&cashBean.bank_name='+encodeURI(encodeURI(bankName.value)),
				type:'POST',
				success:function (data){
	...
	function cashCert(){
		...
		$.ajax({
				type:'post',
				url:'/chlw/user/tuserinfoAction!cashCert',
				data:params,//参数
				success:function (data){
					if(data=='请您登录'){
						openAlert(data);
						...
						window.location.href='/rules/user.html?key=11';
					}else if(data!='cash'){
						openAlert(data);
						//window.location.href='/rules/user.html?key=11';
					}
	...
	function cancelCash(){
		...
		$.ajax({
				type:'post',
				url:'/chlw/user/tuserinfoAction!cancelCash',
				data:params,//参数
				success:function (data){
					if(data=='erroe'){
						openAlert("撤销提现失败！");
	...
	//开奖公告页面单击期号查开奖信息
	 function getOnedrawalottery (divID){
		...
		$.ajax({
			type:'POST',
			url:'/chlw/function/selectAll!getOnelottery?lotno='+lotno+"&batchcode="+batchcode,
			dataType:'json',
	 ...
	 function checkpass(){
			...
			if(oldpass.length!=Trim(oldpass).length){
				...
				var img = $(node>'img');
				if(img.length<=0){
					$("#oldimages").append("<img  src='/images/icon_chahao.gif'/>");
				...
				var params=$('#form2').serialize();
				$.ajax({
				url:'/chlw/user/tuserinfoAction!changeoldpassword',
				type:'POST',
				data:params,
				datatype:'string',
				success:function(data){		
					...
					if(data=='true') {
						...
						var img = $(node>'img');
						if(img.length<=0){
							$("#oldimages").append("<img src='/images/lvduigou.gif'/>");
							$("#oldpromt").text("");
							a=1;
						}
						return true;
		 ...
		 function checknewpass(){
			...
			if(newpas!=''){
				 if(newpas.length<6 || newpas.length>16){
					$("#newpassimage").append("<img src='/images/icon_chahao.gif'>");
					...
					$("#newpassimage").append("<img src='/images/lvduigou.gif'>");
					$("#newpromt").text("");
					b=1;
					return true;
				}
	...
	function updataPassword(){
		if(a==1 && b==1 && c==1){
			var param = $("#form2").serialize();
			 $.ajax({
				url:'/chlw/user/tuserinfoAction!changePassword',
				type:'POST',
				data:param,
				datatype : 'string',
				...
				success:function (data){
					if(data=="changePasswordSuccess"){
						$("#formId").html('<div class="alter_content" id="successId"><div class="alter_tu"><span><img src="/images/duigou.gif" /></span><span class="mimatishi"><img src="/images/mimatishi.gif" /></span></div><div class="mima_zi">'+decodeURI(EncodeUtf8("请牢记您的登录密码。"))+'<a href="#" onclick="ajaxOutToLogin();"> '+decodeURI(EncodeUtf8("重新登录"))+'</a></div>');
					}else{
					 openAlert(data);}
				}
		 });
	...
	function jumpurl(){
		...
		/*if(real_name==null||real_name==""){
			openAlert(decodeURI(EncodeUtf8("您还没有绑定真实姓名，为了您的提款安全，请您先绑定真实姓名！")));
			return;
		}*/
		if(passwordMark!=1){
			openAlert(decodeURI(EncodeUtf8("登陆密码输入不一致，请确认后重新输入！")));
			return;
		}
		if(!checkMoney() || !checkBankNO()||!checkBankName()||!checkName_real_new()){
		   return;
		}
		...
		for(b in $.browser) if(b=='msie'){params=encodeURI(params)+"&msie=1";};
		$.ajax({
			url	: '/chlw/user/tuserinfoAction!getParam?'+params,
			type:"POST",
			data:params,
			success:function(data){
				if(data!="cash_next"){
	...
	if(newpass==new_pass){
		...
		if(new_pass.length<4 || new_pass.length>16){
			$("#realimages").append("<img src='/images/icon_chahao.gif'>");
			...
			$("#realimages").append("<img src='/images/lvduigou.gif' />");
			$("#realpromt").text("");
			c=1;
			return true;
		}
 ...
 function CustomMessage_() {
	//判断用户是否登录       
		...
		if($("#useremail").val().length>0){
			...
			if(!mailPattern.test($("#useremail").val())){
				openAlert(decodeURI(EncodeUtf8("您输入的邮箱格式不正确，请重新输入！")));
				return false;
			}
		}
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data.indexOf("{success}")==-1){
						...
						window.location.href="/rules/customMessageSuccess.html";
					}
				}		
			});
 ...
 function CustomMessage() {    
		var re = /^[\d]+$/;
			...
			if($("#userphone").val().length>0){
				...
				// 发送请求
				$.ajax({
					url	: '/chlw/function/custom!newCustomMessage',
					type:"POST",//数据发送方式
					data:params,//参数
					dataType:'html',//接受数据格式
					success:function(data){
 ...
 function getList(formId, divId) {
		...
		$.ajax({
			url: "/chlw/function/custom!getMsgList",
			//后台处理程序
			type: "post",
			//数据发送方式
			data: parameters,
 ...
 function channelKJ(id,param){	
		...
		$.ajax({
				url	: '/chlw/function/tuserinfoAction!setSMS?smstype='+param+'&state='+state,
				type:'POST',//数据发送方式
				success:function(data){					
					if(data=="success"){
	...
	function selectSMS(id,param){
		$.ajax({
			url : '/chlw/function/tuserinfoAction!findSMS?smstype='+param+'&a='+Math.random(),
			type:'GET',
	...
	function phoneUnBundling(){
		$.ajax({
			url : '/chlw/function/tuserinfoAction!phoneUnBundling',
			type:'post',
...
//	//领取彩票彩票发验证码
//	function SengMsgForPresent(presentid){
//		loginShow("Recvice_Alert",false);
//		var parameters ="presentid="+presentid;
//		$.ajax({
//			url : '/chlw/function/selectAll!sendRandomSMS',
//			type:'post',
//			//数据发送方式
//			data: parameters,
//			async: false,
...
//				var msg=data.split("_")[0];
//				$("#PresentId").val(id);
//				if(msg=="UNLOGIN"){
//					openWindow('您未登录，请先登录！');
//					loginRequrl();
//				}else if(msg=="NONID"){
//					openWindow("为获取方案编号，请刷新后再试！");
//				}else if(msg=="FAIL"){
//					$("#inputyzmTip").val("发送验证码失败,点击重试！");
//					$("#count").html("0");
//					loginShow("Recvice_Alert",false);
//				}else{
...
//	}
//	//领取彩票彩票发验证码
//	function recivePresent(){
//		if($("#inputyzm").val()==""){
//			$("#count").html("0");
//			openWindow('请输入验证码！');
//			return false;
//		}
//		var parameters ="presentid="+$("#PresentId").val()+"&randomCode="+$("#inputyzm").val();
//		$.ajax({
//			url : '/chlw/function/selectAll!recivePresent',
//			//数据发送方式
//			data: parameters,
//			type:'post',
//			async: false,
...
//					openWindow("为获取方案编号，请刷新后再试！");
//				}else if(data=="FAIL"){
//					$("#inputyzmTip").html("验证码错误或已过期，请重试");
//					$("#count").html("0");
//				}else{
//					$("#bodys").toggle();
//					$("#showmsg").toggle();
