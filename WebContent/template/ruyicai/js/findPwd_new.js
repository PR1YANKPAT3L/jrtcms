...
function checkFindMethod_Email(){
	...
	if(radioOne=='checked'){
		...
		if(reg.test(email)){
			//传到action中比对email
			$.ajax({
				url:'/chlw/function/tuserinfoAction!checkUserInfo?param='+email,
				type:'POST',
				async: false,
				dataType:'text',
				success:function(data){
...
function checkFindMethod_phone(){
	...
	if(radiotwo=='checked'){
		...
		if(reg.test(mobile)){
			//传到action中比对mobile
			$.ajax({
				url:'/chlw/function/tuserinfoAction!checkUserInfo?param='+mobile,
				type:'POST',
				async: false,
				dataType:'text',
				success:function(data){
...
function findPwd_next(){
	...
	if(checkname==1 && (checkemail==1 || checkphone==1)){
		...
		$.ajax({
			url:'/chlw/function/tuserinfoAction!sendYZmessage?yzm='+yzm,
			type:'POST',
			dataType:'text',
			success:function(data){
...
function tGPhoneCheck(url) {
	...
	$.ajax({
		url	: '/chlw/function/findtuser2!ryc_checkPhoneRand',
		type:"POST",
		data:params,
		dataType:'text',
		async: false, 
...
function tGPhoneCheckPwd(url) {
	...
	if(checkNewPass() && checkRealPass()){
		...
		$.ajax({
			url	: '/chlw/function/findtuser2!ryc_resetPwdByPhone',
			type:"POST",
			async:false, 
			data:params,
			dataType:'text',
			...
			success:function(data){
				if(data==1){
					...
					openAlert(data,"Shouye_Alert");
					window.location.href="/rules/findPwd_new.html";
				}
			}		
		});
	}else{
...
function getSendMessageAgain(url){
	if($("#count").text()==0){
		...
		$.ajax({
			url : '/chlw/function/findtuser2!ryc_sengmsgByphone_new',
			type:"POST",//数据发送方式
...
function getSendEmail(){
	if($("#count").text()==0){
		$.ajax({
			url : '/chlw/function/findtuser2!ryc_sendMessageByEmail_new',
			type:"POST",
			dataType:'text',
			async: false,
			success:function(data){
...
function tGPhoneCheckPwdByEmail(url) {
	if(checkNewPass() && checkRealPass()){
		var params=$('#updatePasswordForm').serialize(); 
		$.ajax({
			url	: '/chlw/function/findtuser2!ryc_resetPwdByEmail',
			type:"POST",
			async:false, 
			data:params,
			dataType:'text',
