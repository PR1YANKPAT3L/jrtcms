...
function checkZhumaShouY() {
	...
	if (reg.exec(zhuma)) {
		...
		var beishu = $("#tb_Multiple").attr("value", beishu);
		if (reg2.exec(beishu)) {
...
function getZCBatchCode(lotNo) {
	$.ajax({
		type: "POST",
		url: "/chlw/function/zucai!getCurrForJsp",
		data: "lotNo=" + lotNo,
		dataType: 'json',
		//接受数据格式
		async: false,
...
function isLogin() {
	var check = "";
	$.ajax({
		url: basepath + "/function/query/isLogin.jsp",
		//后台处理程序
		type: "POST",
		//数据发送方式
		async: false,
...
function getJiangChi(lotno, divid) {
	$.ajax({
		url: '/chlw/function/selectAll!getlotteryProgressive?lotno=' + lotno,
		type: 'post',
		//数据发送方式   
		async: false,
		dataType: 'json',
...
//---------------------------------------------合买js------------------------------------
//ajax请求首页的合买推荐
//function ajaxshouyeToHemaiCenter(divId) {
//	var parameters = "pageCount=10&isAjax=shouye";
...
function getdrawalotteryFTInfo(divID) {
	...
	window.location.href = '/chlw/function/ryc_select_newkj!getOnelotteryinfo?lotno=' + lotno + "&batchcode=" + batchcode;
...
function getdrawalotteryGPInfo(divID) {
	var lotno = $("#lotno").val();
	var time = $("#" + divID).val();
	window.location.href = '/chlw/function/ryc_select_newkj!drawalotteryInfo?lotno=' + lotno + "&time=" + time;
...
function getdrawalotteryGPInfoM(issnum, divID) {
	var lotno = $("#lotno").val();
	var time = $("#" + divID).val();
	window.location.href = '/chlw/function/ryc_select_newkj!drawalotteryInfo?lotno=' + lotno + "&issu=" + issnum + "&time=" + time;
}
//-----------------开奖详情js-------------------------
...
function getObjForNameAndValueToKey(key) {
	if (isLogin()) { //判断session是否失效 如果失效跳转到登陆页面
		...
		reHtml(key, flowno + "&" + play_name + "&" + batchcode+"&"+type, false, '');
	} else {
		loginRequrl();
...
function phoneChange(name) {
	...
	case "nokia":
		...
		$("#phoneNum").attr({
			"src": "/images/nokia/nokia_3230_j.jpg",
			"alt": decodeURI(EncodeUtf8("诺基亚")),
			"title": decodeURI(EncodeUtf8("诺基亚"))
		});
	...
	case "motorola":
		...
		$("#phoneNum").attr({
			"src": "/images/motorola/motorola_A853_a.jpg",
			"alt": decodeURI(EncodeUtf8("摩托罗拉")),
			"title": decodeURI(EncodeUtf8("摩托罗拉"))
		});
	...
	case "SonyEricsson":
		...
		$("#phoneNum").attr({
			"src": "/images/SonyEricsson/SonyEricsson_C907_a.jpg",
			"alt": decodeURI(EncodeUtf8("索尼爱立信")),
			"title": decodeURI(EncodeUtf8("索尼爱立信"))
		});
	...
	case "samsung":
		...
		$("#phoneNum").attr({
			"src": "/images/samsung/samsung_behold2_a.jpg",
			"alt": decodeURI(EncodeUtf8("三星")),
			"title": decodeURI(EncodeUtf8("三星"))
		});
	...
	case "htc":
		...
		$("#phoneNum").attr({
			"src": "/images/htc/htc_3300_a.jpg",
			"alt": decodeURI(EncodeUtf8("HTC")),
			"title": decodeURI(EncodeUtf8("HTC"))
		});
	...
	case "acer":
		...
		$("#phoneNum").attr({
			"src": "/images/acer/acer_Liquid_a.jpg",
			"alt": decodeURI(EncodeUtf8("宏基")),
			"title": decodeURI(EncodeUtf8("宏基"))
		});
	...
	case "lenovo":
		...
		$("#phoneNum").attr({
			"src": "/images/lenovo/lenovo_lephone_a.jpg",
			"alt": decodeURI(EncodeUtf8("联想")),
			"title": decodeURI(EncodeUtf8("联想"))
		});
	...
	case "huawei":
		...
		$("#phoneNum").attr({
			"src": "/images/huawei/huawei_C8600_a.jpg",
			"alt": decodeURI(EncodeUtf8("华为")),
			"title": decodeURI(EncodeUtf8("华为"))
		});
	...
	case "LG":
		...
		$("#phoneNum").attr({
			"src": "/images/LG/LG_E720OptimusChic_a.jpg",
			"alt": decodeURI(EncodeUtf8("LG")),
			"title": decodeURI(EncodeUtf8("LG"))
		});
	...
	case "others":
		...
		$("#phoneNum").attr({
			"src": "/images/others/others_Dell_Mini3i_a.jpg",
			"alt": decodeURI(EncodeUtf8("其他品牌")),
			"title": decodeURI(EncodeUtf8("其他品牌"))
		});
...
function phoneNumchange(num) {
	...
	case $("#mobileproducer").find("option:selected").text():
		$("#phoneNum").attr({
			"src":
			"/images/" + $("#mobileproducer").val() + "/" + $("#mobileproducer").val() + "_" + $("#machineinfo").val() + ".jpg",
			"alt": $("#machineinfo").find("option:selected").text(),
			"title": $("#machineinfo").find("option:selected").text()
		});
...
function judgeClienttyp(phoneNum) {
	if ($("#machineinfo").val().indexOf("_j") > -1) {
		...
		case "C7-00_j":
		case "X7-00_j":
			$("#client").html(decodeURI(EncodeUtf8("java版客户端")));
			$("#downLoad").attr("href", "/downLoad/ruyicai_5800.jar");
			break;
		case "6600_j":
		case "6700S_j":
		case "E50_j":
		...
		case "E90_j":
			$("#client").html(decodeURI(EncodeUtf8("java版客户端")));
			$("#downLoad").attr("href", "/downLoad/ruyicai_N7370.jar");
			break;
		case "6760S_j":
		case "7610_j":
		case "E52_j":
		...
		case "E63_j":
			$("#client").html(decodeURI(EncodeUtf8("java版客户端")));
			$("#downLoad").attr("href", "/downLoad/ruyicai_E71.jar");
			break;
		default:
			$("#client").html(decodeURI(EncodeUtf8("java通用版")));
			$("#downLoad").attr("href", "/downLoad/ruyicai_N73.jar");
			break;
		}
	} else if ($("#machineinfo").val().indexOf("_a") > -1) {

		$("#client").html(decodeURI(EncodeUtf8("Android 2.6正式版")));
		$("#downLoad").attr("href", "/downLoad/ruyicai_android_2.6_533_wapguanwang.apk");
	}
}
