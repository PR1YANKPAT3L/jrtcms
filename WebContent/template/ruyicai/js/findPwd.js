...
function jumpUrl(url){
	...
	$.ajax({
		url	: '/chlw/function/findtuser2!ryc_FindPwdWays',
		type:"POST",
		data:params,
		dataType:'text',
		//async: false, 
...
function checkRand(){
	$.formValidator.initConfig({
		formid:"callbackForm",
		onError:function(){
	openAjaxPopup("openAlert",1000,"/rules/alert.html?tishi='校验没有通过，具体错误请看错误提示。'");
		}
	});
	...
	$.ajax({
		...
		success:function(data){
			if(data==1){
				...
				openAlert(data);
				window.location.href="/rules/findPwd.html";
			}
		}		
	});
...
function getSendMessageAgain(url){
	...
	$.ajax({
		url : '/chlw/function/findtuser2!ryc_sengmsgByphone',
		type:"POST",//数据发送方式
...
function getSendEmail(){
	notRepeat('findPwdinput','givesend_bottom','');
	$.ajax({
		url : '/chlw/function/findtuser2!ryc_sendMessageByEmail',
		type:"POST",
		dataType:'text',
		async: false,
		success:function(data){
