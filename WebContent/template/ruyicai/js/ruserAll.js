...
//验证用户名是否可用
  ...
  //bl 为true  表示需要用户存在    为false 表示不需要这个用户名存在
	function checkName(id,bl){
		...
		$.ajax({
			url:'/chlw/function/tuserinfoAction!queryUserName',
			type:"POST",
			data:params,
			async:false,
			dataType:'text',
...
function checkPassword(uid,pid){
	...
	if(username==null || username == "undefind" || username ==""){
		...
		$.ajax({
			type:"get",
			url:"/chlw/function/tuserinfoAction!getPasswordByUserName?username="+username+"&reg_password="+password,
			datatype:'html',
			success:function(data){
				if(data=="true"){
...
function selectToAction(){
	...
	if(radiotwo == true){
		...
		if(passflag==1){
			$.ajax({
				type:'post',
				url:'/chlw/function/unitedLogin!userInfoBind?username='+username+"&password="+password,
				async:false,
				success:function (data){
...
function registerUser(name,password,inputyzm){
	...
	$.ajax({
		type:'post',
		url:'/chlw/function/unitedLogin!registerUser',
		async:false,
		data:parameters,
		success:function (data){
			if(data=="success"){
				...
				$.ajax({
				type:'get',
				url:'/chlw/function/unitedLogin!userBindInfo',
				async:false,
				...
				success:function(data){
					if(data=="success"){
						...
						window.location.href='/chlw/function/user/setNickName_logistics.jsp';
					}else{
						window.location.href=data;
					}
				}
...
function setNickName(){
	...
	$.ajax({
		type:'post',
		url:'/chlw/function/tuserinfoAction!appsetNickName',
		async:false,
		data:parameters,//参数
		success:function (data){
			if(data=="success")
			...
			success:function(data){
					...
					if(data.indexOf("{success}")==-1){
						...
						if(data.indexOf("{nickname}") != -1){
							...
							window.location.href="/rules/user.html?key=23&view=ChildMenu1&style=menu1";
						}
...
//找回密码步骤验证，没有则进入默认的页面
 ...
 // alert(decodeURI(EncodeUtf8("操作非法！请按照正常的流程操作！")));
   window.location.href="/rules/findPwd_new.html?"+url;
  }
 }
...
function passStrength(){
 ...
 function phoneBandCheck(){
	 ...
	 if(pageFlashNum==0){
			pageFlashNum++;
	$.ajax({
		url : '/chlw/user/tuserinfoAction!phoneBandSendMessage?usermobile='+mobile,
		type:"POST",//数据发送方式
		dataType:'html',//接受数据格式
		success:function(data){
			sendCount=data;
 ...
 function getUserScore(divid){
	$.ajax({
		url : '/chlw/user/tuserinfoAction!getUserScore',
		type:"POST",//数据发送方式
		async:false,
		dataType:'json',
		success:function(data){
 ...
 function makescoreToM(divid){
	 ...
	 var parameters ="scoreNum="+$("#"+divid).val();
		$.ajax({
			url : '/chlw/user/tuserinfoAction!transScore2Money',
			type:"POST",//数据发送方式
			data: parameters,
			async: false,
			//参数
