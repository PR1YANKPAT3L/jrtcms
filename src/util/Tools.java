...
public class Tools {
	 ...
	 */
	public static boolean makeHtml(String pageUrl, String filePath, String fileName) {
		// 调用核心方法
		return makeHtml(pageUrl, filePath, fileName, "UTF-8");
	 ...
	 */
	public static boolean makeHtml(String pageUrl, String filePath,
...
//		synchronized (lock) {
			...
			try {
				...
				bufferedWriter = new BufferedWriter(new OutputStreamWriter(
						...
						chartset));// 设置写文件的路径和文件名称
				bufferedReader = new BufferedReader(new InputStreamReader(
						stream, chartset));// 获取输入流
				String line;
				...
				log.info("call jsp address success ："+pageUrl);
				log.info("generate html address success ："+filePath + fileName);
				return true;
			} catch (Exception e) {
				log.error("call jsp address error ："+pageUrl);
				return false; 
			} finally {
				try {
